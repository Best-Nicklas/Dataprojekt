...
LTFH <- function(rds.obj, prevalence, h2) {
  ...
  child_configs <- purrr::pmap(list(rds.obj$FAM$Status, rds.obj$FAM$p1_Status, rds.obj$FAM$p2_Status, rds.obj$FAM$sibs_Status), 
                        function(x1, x2, x3, x4) paste(toString(x1),
                                                       toString(max(x2, x3)),
                                                       toString(min(x2, x3)),
                                                       strrep(1, sum(x4)),
  ...
  #Calculates the the posterior mean genetic liability for each unique config
  config_liabs <- vector()
  ...
  #Matches config of child to a calculated posterior mean genetic liability
  for (i in 1:n) {
    gen_liabs[i] <- config_liabs[child_configs[i]]
  }
  
  return(list(GWAS_Data = GWAS(rds.obj, gen_liabs), 
              Posterior_Mean_Genetic_Liability = gen_liabs, 
              Configs = data.frame(Config = unique_configs, 
                                   Liability = config_liabs)))
}
