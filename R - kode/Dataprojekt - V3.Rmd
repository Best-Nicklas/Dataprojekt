...
>>>>>>> 0b21bd77ba427cb59aafaab3595de72e1efb1710

Conditional_calculater <- function(liab, i, covmatrix) {
  s11 <- covmatix[i, i]
  s12 <- matrix(covmatix[i, -c(i)], nrow = 1)
  s21 <- matrix(covmatix[-c(i), i], ncol = 1)
  s22 <- covmatix[-c(i), -c(i)]
...
<<<<<<< HEAD
  new_mu <- s12 %% solve(s22) %% liab[-i]   # Vi mener du sagde noget om, at vi kunne fjerne mu1 og mu2, men vi er ikke sikre
  new_sigma <- s11 - s12 %% solve(s22) %% s21
...
mean_liabs <- numeric(6)
# Gibbs sampling for hver konfig
for (i in 1:6) {
  config <- configurations[i,]
  l_all <- c(rnorm(4,0,1))  #initialize l 
  
  mean <- NA
  sigma <- NA 
  
  # conv - antal kørsler indtil konvergens 
  for (j in 1:conv) {
    
    # iteration for hver parameter
    for (p in 1:length(l_all)) {
      temp <- Conditional_calculater(l_all, p, cov_mat)
      mean <- temp[1]
      sigma <- temp[2]
      
      if (config[] == 0) {
        l_all[p] <- rnorm(mean, sqrt(sigma))  
      }
      
      else {
        crit_value <-  pnorm(qnorm(1-alpha, mean, sqrt(sigma)), mean, sqrt(sigma)) 
