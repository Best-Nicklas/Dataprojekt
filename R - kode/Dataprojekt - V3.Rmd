...
covariance_matix <- matrix(c(1, 0.5, 0.5, 0.5, 0.5, 1, 0, 0.5, 0.5, 0, 1, 0.5, 0.5, 0.5, 0.5, 1), nrow = 4, byrow = TRUE)
covariance_matix 

condition <- function(covariance_matix, vector_a, person_index) {
  s11 <- covariance_matix[person_index, person_index]
  s12 <- matrix(covariance_matix[person_index, -c(person_index)], nrow = 1)
  s21 <- matrix(covariance_matix[-c(person_index), person_index], ncol = 1)
  s22 <- covariance_matix[-c(person_index), -c(person_index)]
  s22_inv <- solve(s22)
  
  c_mu <- s12 %*% s22_inv %*% vector_a[-person_index]
  c_covariance_matix <- s11 - s12 %*% s22_inv %*% s21
  
  return(c(c_mu, c_covariance_matix))
}
