...
```{r}
Cond_calc <- function(liab, i, covmatrix) {
  s11 <- covmatrix[i, i]
  s12 <- covmatrix[i, -i]
  s21 <- covmatrix[-i, i]
  s22 <- covmatrix[-i, -i]

  new_mu <- s12 %*% solve(s22) %*% liab[-i]  
  new_sigma <- s11 - (s12 %*% solve(s22) %*% s21)
  # Vi mener du sagde noget om, at vi kunne fjerne mu1 og mu2, men vi er ikke sikre
...
for (i in 1:6) {
  ...
  for (j in 1:conv) {
    ...
    for (p in 1:length(l_all)) {
      temp <- Cond_calc(l_all, p, cov_mat)
      mean <- temp[1]
      sigma <- temp[2]
