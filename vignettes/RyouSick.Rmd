...
Before simulating genotypes and case-control phenotype data using RyouSick, we first need to simulate the disease parameters with the function `sim_disease()`. The disease parameters for our target disorder is simulated on 10 SNPs with a heritability h2 of 0.8 and a prevalence in the population of 0.25:
...
The x axis of the power plots are estimated effect sizes, which are correlated with the p-value for each SNP. A larger numerical estimated effect size for a SNP, will therefore result in a greater chance of the methods analyzing the SNP to be causal. 

The y axis is the cumulative number of SNP´s found causal by the methods. 
